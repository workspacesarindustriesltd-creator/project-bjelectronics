...
test("serves existing static assets without fallback", async () => {
  const calls = [];
  const response = await worker.fetch(new Request("https://bjelectronics.shop/assets/app.js"), { ASSETS: assets(calls) });
...
test("routes storefront navigation to the storefront shell", async () => {
  const calls = [];
  const response = await worker.fetch(new Request("https://bjelectronics.shop/products/laptop", { headers: { accept: "text/html" } }), { ASSETS: assets(calls) });
  assert.equal(await response.text(), "/index.html");
  assert.deepEqual(calls, ["/products/laptop", "/index.html"]);
});

test("routes administrator paths to the separate admin shell", async () => {
  for (const url of ["https://bjelectronics.shop/admin/", "https://bjelectronics.shop/admin/dashboard"]) {
    const calls = [];
...
test("does not convert missing API or write requests into an app shell", async () => {
  for (const request of [
    new Request("https://bjelectronics.shop/api/missing", { headers: { accept: "application/json" } }),
    new Request("https://bjelectronics.shop/flow", { method: "POST", headers: { accept: "text/html" } }),
  ]) {
    const calls = [];
